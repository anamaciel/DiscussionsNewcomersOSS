...
# Importing dataset
dados <- read.csv("RDD-ALLProjects.csv",sep=";")
#dados <- read.csv("testeData9-SemZeros.csv",sep=";")
dados['time'] <- lapply(dados['time'] , factor)

dados$time <- rep((1:24)[1:24 != 0], 285)

dados["month_index"] <- NA
dados$month_index <- rep((-12:12)[-12:12 != 0], 285)
dados$month_index <- factor(dados$month_index, levels=c((-12:12)[-12:12 != 0]),ordered=TRUE)

#any(is.na(dados$time_after_intervention))
#any(is.nan(dados$time_after_intervention))
#any(is.infinite(dados$time_after_intervention))
...
vif(lm(log(new_users_issues + 1) ~ ageIssues
       ...
       + time 
       + intervention
       + time_after_intervention, data=dados))
